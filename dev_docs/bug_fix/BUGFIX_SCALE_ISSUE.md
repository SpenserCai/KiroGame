...
在修复了根本的缩放问题后，我们可以安全地恢复选中时的脉冲动画效果。

### 脉冲动画设计

新的脉冲动画基于 `normalScale` 进行相对缩放：
...
1. **缩小阶段**：从 `normalScale` 缩小到 `normalScale * 0.95`（400ms）
2. **放大阶段**：从 `normalScale * 0.95` 放大到 `normalScale * 1.05`（400ms）
3. **恢复阶段**：从 `normalScale * 1.05` 恢复到 `normalScale`（400ms）
4. **循环**：重复上述过程

### 关键特性

1. **相对缩放**：所有缩放都基于 `normalScale`，不是硬编码值
2. **平滑过渡**：使用 `easeInOutQuad` 缓动函数
3. **安全停止**：每个阶段都检查 `selectionTweens.has(sprite)`，确保动画可以随时停止
4. **状态恢复**：无论动画在哪个阶段停止，都会恢复到 `normalScale`

### 为什么现在可以安全使用

1. **正确的基准值**：`normalScale` 是精灵的正确大小（0.5），而不是错误的 1.0
2. **相对缩放**：脉冲范围是 `0.95x - 1.05x`，相对于 `normalScale`
3. **一致性**：所有动画都使用相同的基准值

### 视觉效果

- 对于 128x128 的纹理显示为 64x64（normalScale = 0.5）：
  - 最小：60.8px（0.5 * 0.95 * 128）
  - 正常：64px（0.5 * 128）
  - 最大：67.2px（0.5 * 1.05 * 128）
  
这是一个微小的、平滑的脉冲效果，不会造成视觉上的突兀。

### 测试要点

1. 选中图标时应该有平滑的脉冲动画
2. 脉冲范围应该是微小的（±5%）
3. 取消选中时动画应该立即停止，精灵恢复到正常大小
4. 快速切换选中不同图标时，动画应该正确切换
