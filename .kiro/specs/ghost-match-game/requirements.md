...
小鬼消消乐是一款基于 PixiJS v8.14.0 的浏览器消除类益智游戏。玩家通过交换相邻的小鬼图标来形成三个或更多相同图标的连线，从而消除它们并获得分数。游戏采用模块化架构设计，使用 PixiJS 作为渲染引擎，通过 npm 管理依赖，确保代码解耦、可扩展且易于维护。Node.js 仅用于开发服务器和资源构建（SVG转PNG），游戏逻辑完全运行在浏览器端。
